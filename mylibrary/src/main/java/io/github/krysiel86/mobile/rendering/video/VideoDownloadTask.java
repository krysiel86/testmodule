...
import io.github.krysiel86.mobile.rendering.video.LruController;
...
//        LogUtil.debug(TAG, "queryParams: " + param.queryParams);
        LogUtil.debug("tpmn", "url: " + param.url);
        LogUtil.debug("tpmn", "queryParams: " + param.queryParams);
    ...
    protected void processData(URLConnection connection, GetUrlResult result) throws IOException {
        ...
        if (file.exists() && !io.github.krysiel86.mobile.rendering.video.LruController.isAlreadyCached(shortenedPath)) {
            LogUtil.debug(TAG, "Video saved to cache");
            ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
            readAndWriteData(connection, result, outputStream, false);
            io.github.krysiel86.mobile.rendering.video.LruController.putVideoCache(shortenedPath, outputStream.toByteArray());
        } else {
    ...
    throws Exception {
        Log.e("tpmn","VideoDownloadTask createResult ");
